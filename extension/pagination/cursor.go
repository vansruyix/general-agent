...
func (c Cursor) Decode(value any) error {
	...
	if err != nil {
		return errorx.ErrRequestParam.WithReason("specify cursor is invalid").Wrap(err)
	}
...
func (p CursorPagination) Validate() error {
	if p.Limit > MAX_PAGE_SIZE {
		return errorx.ErrRequestParam.WithReason(fmt.Sprintf("limit must be less than or equal to %d", MAX_PAGE_SIZE))
	}
	if p.Limit < 0 {
		return errorx.ErrRequestParam.WithReason("limit must be greater than or equal to 0")
	}
