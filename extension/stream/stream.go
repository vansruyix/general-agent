...
import (
	...
	"general-agent/extension/stream/collectors"
	"runtime"
	"sort"
	"sync"
)
...
func newStm[S ~[]T, T any](isParallel bool, s S) Stream[T] {
	source := make(chan T, len(s))
	for _, item := range s {
		source <- item
	}
	close(source)
...
func (s Stream[T]) Find(fn func(item T) bool) Optional[T] {
...
func (s Stream[T]) SumIntStatistics() SumIntStatistics[int] {
	...
	var min int
	for item := range s.source {
		i := objects.ToAny[int](item)
		if cnt == 0 {
			min = i
		}
		cnt++
		sum = sum + int64(i)
		max = objects.If(max > i, max, i)
		min = objects.If(min < i, min, i)
	}
	return SumIntStatistics[int]{
		...
		Max:     max,
		Min:     min,
		Average: objects.If(cnt > 0, float64(sum)/float64(cnt), 0),
	}
}
...
func (s Stream[T]) SumFloat32Statistics() SumFloatStatistics[float32] {
	...
	return SumFloatStatistics[float32]{
		...
		Max:     max,
		Min:     min,
		Average: objects.If(cnt > 0, sum/float64(cnt), 0),
	}
}
...
func (s Stream[T]) MapToUint64(mapper func(T) uint64) Stream[uint64] {
	return Map[T](s, mapper)
}
...
func OfSlice[S ~[]T, T any](s S) Stream[T] {
	return newStm(false, s)
}
func OfSliceParallel[S ~[]T, T any](s S) Stream[T] {
	return newStm(true, s)
}
