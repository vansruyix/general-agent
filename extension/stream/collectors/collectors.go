...
func Statistic[T Number]() Collector[T, *SumStatistics[T], SumStatistics[T]] {
	return &DefaultCollector[T, *SumStatistics[T], SumStatistics[T]]{
		...
		accumulator: func(ts *SumStatistics[T], t T) {
			...
			ts.Count++
			ts.Sum = ts.Sum + t
			ts.Max = objects.If(ts.Max > t, ts.Max, t)
			ts.Min = objects.If(ts.Min < t, ts.Min, t)
			c := objects.ToAny[float64](ts.Count)
			s := objects.ToAny[float64](ts.Sum)
			ts.Average = objects.If(ts.Count > 0, s/c, 0)
		},
