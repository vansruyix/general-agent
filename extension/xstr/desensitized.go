...
/**
 * 【通用脱敏】只显示前1/4和后1/4位，其他用*代替
 * 例如：1234567890 -> 123****890
 *
 * @param str 字符串
 * @return 脱敏后的字符串
 */
func common(str string) string {
	if str == "" {
		return ""
	}
	...
	// 如果前缀和后缀重叠，则只保留前缀和一个星号
	if frontLen+endLen >= length {
		return str[:1] + strings.Repeat("*", length-1)
	}

	// 提取前缀和后缀
	frontPart := str[:frontLen]
	endPart := str[length-endLen:]

	// 计算中间需要替换为*的长度
	middleLen := length - frontLen - endLen

	// 拼接结果
	return frontPart + strings.Repeat("*", middleLen) + endPart
}
